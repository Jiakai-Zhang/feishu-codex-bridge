...
```text
请把这台 Windows 电脑上已有的 Feishu Codex Bridge 安全升级到私有测试 tag v0.4.0-windows-rc.2。运行任何命令前先要求我确认当前 Codex 对话已设为 Full access。只读检查安装目录、操作系统、git status（包括未跟踪文件）、所有 remote、当前精确 tag、Bridge、Desktop relay 已保存的直连/代理模式、App Server PID/进程记录和自建 watchdog/guardian；再用本机已登录的 GitHub CLI 确认账号可访问 ninmon/feishu-codex-bridge-private，并读取该 release 的 AGENTS.md、docs/INSTALL_AGENT.md 与 Release Note。不得输出 Token、配置、身份标识或任务路径。
...
若现有 update.ps1 已支持 Remote 参数，必须运行 .\update.ps1 -Version v0.4.0-windows-rc.2 -Remote private。若旧升级器尚不支持该参数，按照目标 Release Note：先从 private 精确 fetch 该 tag，把该 tag 中的 update.ps1 写入系统临时文件，再以 -InstallRoot 当前安装目录、-Version v0.4.0-windows-rc.2、-Remote private 执行目标 updater。不得用 main、手工 checkout、reset、clean 或 stash 代替。
