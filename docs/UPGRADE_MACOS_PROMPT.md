...
本协议把一台已安装 Feishu Codex Bridge 的 Mac 安全升级到私有固定版本 `v0.4.0-macos-rc.7`。健康路径中，用户只在前台升级器明确准备完成后按一次 `⌘Q` 完全退出 ChatGPT/Codex Desktop；独立 Terminal 会保留原有直连/代理模式，事务升级、自动重开 Desktop 并完成最终 Doctor。不得让用户复制或运行更新命令，也不得要求用户手工重开 Desktop。

本文锁定到 `v0.4.0-macos-rc.7`。后续版本不得沿用本文件中的 tag，必须重新发布并验证协议。
...
```text
请把这台 Mac 上已有的 Feishu Codex Bridge 安全升级到私有固定 tag v0.4.0-macos-rc.7，并完整验收。
...
3. 在改变 checkout 或停止服务前，使用本机已登录的 GitHub CLI/Git 凭据确认目标 tag 已发布并解析到精确提交，然后完整读取该 tag 中的：
   ...
   - docs/releases/v0.4.0-macos-rc.7.md；
...
7. 预检全部通过后，从已认证的 private 精确 tag 提取目标版本的 src/runtime/platform/macos/update-with-desktop-restart.sh 到当前用户的安全临时目录。在现有安装仓库作为工作目录，以目标 tag v0.4.0-macos-rc.7 和 Remote private 启动这个目标版本入口。不得改用当前旧版本脚本、手工 checkout、后台 job、公开 raw URL 或自行拼装 Terminal 命令。
...
9. 前台升级器必须调用目标版本的底层 update.mjs/update.sh 事务：保留私有状态、失败自动回滚；relay 已启用时即使 Bridge 当时停止，也要恢复 Bridge、relay 和 watchdog。更新后先执行严格 Doctor，再通过仓库启动器的 --preserve-network 模式重开 Desktop，最后执行包含 Desktop attachment 的严格 Doctor。失败且 Desktop 已退出时，应尝试按保存的网络模式恢复打开 Desktop，并让 Terminal 保持可见。

10. Desktop 自动重开并回到本任务后，自主核对：
   - git describe --tags --exact-match 精确为 v0.4.0-macos-rc.7；
   - HEAD 与远端该 tag 的提交一致；
   - 工作树干净；
   - ./status-bridge.sh 正常；
...
- 直连和本地代理都通过同一个持久选择恢复；升级调用链不会把代理 URL 写入新的启动命令参数。
