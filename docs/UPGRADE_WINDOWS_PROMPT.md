...
本协议把一台已安装 Feishu Codex Bridge 的 Windows 电脑安全升级到私有固定版本 `v0.4.0-windows-rc.4`。正常健康路径由 Codex 启动目标 release 自带的可见前台升级器；用户只需在明确提示后完全退出 ChatGPT/Codex Desktop，升级器会事务更新、按原直连/代理模式自动重开 Desktop，并完成严格 Doctor。不得要求用户复制、粘贴或运行升级命令。

本文锁定到 `v0.4.0-windows-rc.4`。后续版本不得沿用本文件中的 tag，必须重新发布并验证协议。
...
3. 在不需要用户操作时自主连续执行；
4. 正常路径只在要求用户完全退出 Desktop 时暂停一次，不得把 PowerShell 命令转交用户执行；
5. Full access、GitHub 认证、私有仓库访问、干净工作树或安全状态不满足时，明确说明阻塞项并暂停。

## 可复制 Prompt

```text
请把这台 Windows 电脑上已有的 Feishu Codex Bridge 安全升级到私有固定 tag v0.4.0-windows-rc.4，并完整验收。
...
正常健康路径中，我只负责在你明确提示后完全退出 ChatGPT/Codex Desktop，不负责重新打开，也不执行任何命令。你必须自行完成只读预检，启动目标 release 的 update-windows-with-desktop-restart.ps1，等待它确认前台窗口已经准备好；Desktop 退出后，由该窗口完成事务升级、按原有代理模式自动重开 Desktop 和最终 Doctor。
...
3. 在改变 checkout 或停止服务前，使用本机已登录的 GitHub CLI/Git 凭据确认目标 tag 已发布并解析到精确提交，然后完整读取该 tag 中的：
   ...
   - docs/releases/v0.4.0-windows-rc.4.md；
...
7. 预检全部通过后，从已经验证的 private 精确 tag 提取 update-windows-with-desktop-restart.ps1 到当前用户临时目录，并以目标 tag v0.4.0-windows-rc.4、当前安装目录和 Remote private 调用它。不得改用当前旧版本的临时 monitor、后台 job、Start-Process 子进程、main、手工 checkout、公开 raw URL、reset、clean 或 stash。
...
8. 前台升级器准备完成后，只要求我执行一个动作：
   “前台升级预检已通过。请现在完全退出 ChatGPT/Codex Desktop，不要自己重新打开；可见 PowerShell 会完成升级、按原网络模式自动重开 Desktop 并验收。”

   不得自行杀死 Desktop，不得要求我回复后再关，不得要求我运行 PowerShell。Desktop 退出后，当前 Codex 任务暂时中断是正常现象；一次性 Scheduled Task 必须独立继续运行。

9. 前台升级器必须按顺序完成：等待所有 Desktop GUI 进程退出；运行目标 release 的事务 update.ps1；核对精确 tag、HEAD 和干净工作树；运行严格 Doctor；用 launch-codex-desktop-with-relay.ps1 显式传回升级前保存的 -Proxy 或 -NoProxy；自动重开 Desktop；再次运行严格 Doctor 并写入 completed 状态。思考过程、临时监控进程或当前 Codex 进程都不能承担这个生命周期。

10. Desktop 自动重开并能继续本任务后，读取前台升级状态并再次安全核对精确 tag、HEAD、git status、status-bridge.ps1 和 doctor.ps1 -RequireRunning -RequireDesktopRelay，确认版本为 v0.4.0-windows-rc.4、Bridge/Channel/共享 App Server/watchdog/Desktop relay 健康，且原有代理模式未改变。只有这些都通过才能报告升级成功。
