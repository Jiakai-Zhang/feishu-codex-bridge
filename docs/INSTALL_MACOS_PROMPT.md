...
本文锁定到 `v0.4.0-macos-rc.7`。它保留 rc.6 的完整多用户与 Session 权限能力，并把全新安装和日后升级统一为固定版本短入口；日后升级由目标版本前台升级器保留网络模式、自动重开 Desktop 并验收。完整要求继续保存在本文件和 `docs/UPGRADE_MACOS_PROMPT.md` 中。后续 RC 不会自动改变本文中的安装目标；使用其他版本时必须先更新并重新验证整份协议。
...
2. 让用户确认仓库的最终存放位置，并确认本机 GitHub CLI 已登录且当前账号有私有仓库访问权，再克隆仓库并检出精确 tag v0.4.0-macos-rc.7。
