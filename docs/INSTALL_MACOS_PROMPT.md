...
本文锁定到 `v0.4.0-macos-rc.3`。它保留 rc.12 的 Full access 停点、Keychain 诊断、浏览器备用 URL、watchdog 驻留重试、飞书直连校验隔离与完整升级回滚，同步 Windows v0.4 的可选多用户 Project 目录、Session 共享权限、多人群 queue/steer 和按发送者隔离的附件草稿，在登记成员后主动发送 Bot 私聊欢迎消息和 `/add` 指引，并允许从 `/add` 在任意非空 Project 中继续创建及绑定新 Session。后续 RC 不会自动改变本文中的安装目标；使用其他版本时必须先更新并重新验证整份协议。
...
2. 让用户确认仓库的最终存放位置，并确认本机 GitHub CLI 已登录且当前账号有私有仓库访问权，再克隆仓库并检出精确 tag v0.4.0-macos-rc.3。
...
17. 在绑定群完成真实验收：
    ...
    如果第 11 步启用了多用户，还必须先由用户把一名测试成员加入应用可用范围并完成必要的发布/审批，再由 Owner 使用 `/members add` 登记。确认 Bot 主动私聊测试成员并显示 `/add` 指引，再实测 Owner 与成员的 `/add` 列表隔离、成员目录内新建 Project/任务、在已有非空 Project 中继续新建并绑定 Session、把成员加入绑定群后的 Session 共享、多人群 `@Bot`/queue/`/steer` 权限、按发送者隔离的附件草稿，以及未登记成员加群时的 fail-closed。成员身份和本机目录不得输出到聊天或日志。
