...
function appServerError(method, error) {
  const result = new Error(`Codex App Server ${method} failed: ${String(error?.message || "unknown error")}`);
  result.name = "CodexAppServerRpcError";
  result.code = "codex_app_server_error";
  return result;
}
...
export class CodexSessionObserver {
  ...
  async stop() {
    ...
    if (connection) {
      ...
      connection.rejectPending(new Error("Codex session observer stopped"));
  ...
  async #connect() {
    ...
    const send = (message) => {
      if (closed) throw new Error("Codex session observer connection is closed");
    ...
    const opened = new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        const error = new Error("Timed out connecting the Codex session observer");
      ...
      socket.addEventListener("error", () => {
        clearTimeout(timer);
        const error = new Error("Could not connect the Codex session observer");
        error.name = "CodexAppServerConnectError";
        reject(error);
      }, { once: true });
    });
    ...
    socket.addEventListener("message", (event) => {
      ...
      try { message = JSON.parse(String(event.data)); }
        ...
        this.log(`Codex session observer ignored invalid JSON: ${error instanceof Error ? error.name : "unknown"}`);
        return;
      }
      if (message?.id !== undefined && pending.has(message.id)) {
        ...
        clearTimeout(entry.timer);
        if (message.error) entry.reject(appServerError(entry.method, message.error));
        else entry.resolve(message.result);
        return;
      }
      if (message?.method && message?.id !== undefined) {
        try {
          send({ id: message.id, error: { code: -32601, message: `Unsupported observer request: ${message.method}` } });
        } catch {}
        return;
      }
      if (message?.method) this.collector.handleNotification(message.method, message.params || {});
    });
    socket.addEventListener("close", (event) => {
      if (closed) return;
      closed = true;
      const error = new Error(`Codex session observer connection closed (code ${event.code})`);
