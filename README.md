...
- Bridge 始终同步绑定任务的所有 Codex 最终答案；**新安装默认开启公开进度与最终回答 @提醒**。开启公开进度后，每个 Turn 只创建一张可更新卡片：公开阶段说明到达时在原消息内追加并刷新“已处理”时长，完成后由最终答案原位替换进度，不再为长任务周期性新增卡片；卡片更新失败时才回退为普通进度消息或持久最终投递。Bridge 只实时转发 App Server 明确标记为 `agentMessage.phase=commentary` 的公开阶段说明，隐藏思维链、`reasoning`/raw reasoning、工具原始输出始终禁止转发；公开进度始终不 @，最终卡片之后可按设置发送一次完成 @提醒和原生附件。可用 `/settings progress off` 关闭公开进度，用 `/settings mention off` 关闭当前 Session 的最终回答 @提醒。断线期间的公开进度不补发，最终答案仍走持久发件箱。一个 App Server turn 是唯一的最终回复与幂等边界，初始 Prompt 和后续任意多次调整都按 App Server 接受顺序组成输入事件流。只要该 turn 含有飞书输入，最终答案就回复其中最后一条飞书消息；完全没有飞书输入时，Bot 才在绑定群主动新发富文本。两种最终回复底部都显示回答完成时间、整轮用时和本轮真实 Token；本轮 Token 由 App Server 的会话累计 usage 差值计算，覆盖一次 turn 内的多次模型调用。断线补发缺少 usage 快照时明确显示“暂不可用”，不做文本长度估算。
